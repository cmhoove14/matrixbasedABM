---
title: "CVOID19 Epi Dynamics In Prisons"
author: "Chris Hoover"
date: "1/11/2021"
...
```{r plot_sp_data, fig.width=8, fig.height=7, fig.cap="New confirmed cases among residents estimated as daily change in reported cumuative cases"}
uclabb_sps %>% 
  ggplot() +
    geom_line(aes(x = Date, y = new_residents_confirmed, col = Name)) +
    theme_bw() +
    theme(legend.position = "bottom",
          legend.text = element_text(size = 6)) +
    guides(col = guide_legend(ncol = 3)) +
...
pop_case_merge <- pop_sps %>% 
  ...
  left_join(uclabb2, by = c("Facility_case" = "Name",
                            "Report_Date" = ))
```
